  ...
  function checkDoMoreWork() {
    ...
    if (directionIsToS3) {
      if (!localFileStat) {
        ...
        !compareMultipartETag(s3Object.ETag, localFileStat.multipartETag)
      ) {
    ...
    function downloadS3Object() {
      ...
      function haveS3Params(err, s3Params) {
        ...
        upDownFileParams.s3Params = extend(
          extend({}, baseUpDownS3Params),
          s3Params
        );
        startDownload();
      }
...
function getPublicUrl(bucket, key, bucketLocation, endpoint) {
  var nonStandardBucketLocation =
    bucketLocation && bucketLocation !== "us-east-1";
  var hostnamePrefix = nonStandardBucketLocation
    ? "s3-" + bucketLocation
    : "s3";
  var parts = {
    protocol: "https:",

...
const MultipartETag = require("./multipart-etag");
...
exports.getPublicUrl = utils.getPublicUrl;
exports.getPublicUrlHttp = utils.getPublicUrlHttp;

exports.Client = Client;
exports.MultipartETag = MultipartETag;
exports.AWS = AWS;

exports.MAX_PUTOBJECT_SIZE = constants.MAX_PUTOBJECT_SIZE;
exports.MAX_DELETE_COUNT = constants.MAX_DELETE_COUNT;
exports.MAX_MULTIPART_COUNT = constants.MAX_MULTIPART_COUNT;
exports.MIN_MULTIPART_SIZE = constants.MIN_MULTIPART_SIZE;
...
function Client(options) {
  ...
  this.multipartUploadThreshold =
    options.multipartUploadThreshold || 20 * 1024 * 1024;
  this.multipartUploadSize = options.multipartUploadSize || 15 * 1024 * 1024;
  this.multipartDownloadThreshold =
    options.multipartDownloadThreshold || 20 * 1024 * 1024;
  this.multipartDownloadSize =
    options.multipartDownloadSize || 15 * 1024 * 1024;

  if (this.multipartUploadThreshold < constants.MIN_MULTIPART_SIZE) {
    throw new Error("Minimum multipartUploadThreshold is 5MB.");
  }
  if (this.multipartUploadThreshold > constants.MAX_PUTOBJECT_SIZE) {
    throw new Error("Maximum multipartUploadThreshold is 5GB.");
  }
  if (this.multipartUploadSize < constants.MIN_MULTIPART_SIZE) {
    throw new Error("Minimum multipartUploadSize is 5MB.");
  }
  if (this.multipartUploadSize > çMAX_PUTOBJECT_SIZE) {
...
Client.prototype.deleteObjects = function (s3Params) {
  ...
  var params = {
    ...
    Delete: utils.extend({}, s3Params.Delete),
    MFA: s3Params.MFA,
  };
  var slices = utils.chunkArray(params.Delete.Objects, constants.MAX_DELETE_COUNT);
...
Client.prototype.uploadFile = function (params) {
  ...
  uploader.getPublicUrl = function () {
    return utils.getPublicUrl(
      ...
      self.s3.config.endpoint
    );
  };
  uploader.getPublicUrlHttp = function () {
    return utils.getPublicUrlHttp(
      s3Params.Bucket,
      s3Params.Key,
      self.s3.config.endpoint
    );
  ...
  var s3Params = utils.extend({}, params.s3Params);
  if (s3Params.ContentType === undefined) {
    var defaultContentType =
      params.defaultContentType || "application/octet-stream";
  ...
  function startPuttingObject() {
    if (localFileStat.size >= self.multipartUploadThreshold) {
      ...
      var partsRequiredCount = Math.ceil(
        localFileStat.size / multipartUploadSize
      );
      if (partsRequiredCount > constants.MAX_MULTIPART_COUNT) {
        multipartUploadSize = smallestPartSizeFromFileSize(localFileStat.size);
      }
      if (multipartUploadSize > constants.MAX_PUTOBJECT_SIZE) {
        var err = new Error(
          "File size exceeds maximum object size: " + localFile
        );
        err.retryable = false;
        handleError(err);
        return;
  ...
  function startMultipartUpload(multipartUploadSize) {
    doWithRetry(
      ...
      function (err, data) {
        ...
        s3Params = {
          Bucket: s3Params.Bucket,
          Key: utils.encodeSpecialCharacters(s3Params.Key),
  ...
  function makeUploadPartFn(start, end, part, uploadId) {
    ...
    function tryUploadPart(cb) {
      ...
      self.s3Pend.go(function (pendCb) {
        ...
        self.s3.uploadPart(utils.extend({}, s3Params), function (err, data) {
          pendCb();
          if (fatalError || errorOccurred) return;
          if (err) {
            errorOccurred = true;
          ...
          pend.wait(function () {
            if (fatalError) return;
            if (!utils.compareMultipartETag(data.ETag, multipartETag)) {
              errorOccurred = true;
              uploader.progressAmount -= overallDelta;
    ...
    self.s3Pend.go(function (pendCb) {
      ...
      self.s3.putObject(s3Params, function (err, data) {
        ...
        pend.wait(function () {
          if (fatalError) return;
          if (
            !utils.compareMultipartETag(data.ETag, localFileStat.multipartETag)
          ) {
            cb(new Error("ETag does not match MD5 checksum"));
            return;
          }
  ...
  function doTheDownload(cb) {
    ...
    request.on("httpHeaders", function (statusCode, headers, resp) {
      ...
      if (headers["content-length"] == undefined) {
        ...
        var eTag = utils.cleanETag(headers.etag);
        var eTagCount = utils.getETagCount(eTag);
        ...
        hashCheckPend.go(function (cb) {
          multipartETag.on("end", function () {
            if (multipartETag.bytes !== contentLength) {
              handleError(
                new Error("Downloaded size does not match Content-Length")
              );
              return;
            }
...
Client.prototype.listObjects = function (params) {
  var self = this;
  var ee = new EventEmitter();
  var s3Details = utils.extend({}, params.s3Params);
  ...
  function findAllS3Objects(marker, prefix, cb) {
    if (abort) return;
    doWithRetry(listObjects, self.s3RetryCount, self.s3RetryDelay, function (
      err,
      data
    ) {
      if (abort) return;
  ...
  finder.on("data", function (objects) {
    ...
    function deleteThem(cb) {
      var params = {
        Bucket: bucket,
        Delete: {
          Objects: objects.Contents.map(utils.keyOnly),
          Quiet: true,
        },
        MFA: mfa,
      };
...
Client.prototype.copyObject = function (_s3Params) {
  var self = this;
  var ee = new EventEmitter();
  var s3Params = utils.extend({}, _s3Params);
  delete s3Params.MFA;
  doWithRetry(doCopyWithPend, self.s3RetryCount, self.s3RetryDelay, function (
    err,
    data
  ) {
    if (err) {
...
Client.prototype.downloadBuffer = function (s3Params) {
  var self = this;
  var downloader = new EventEmitter();
  s3Params = utils.extend({}, s3Params);
...
function syncDir(self, params, directionIsToS3) {
  ...
  var finditOpts = {
    fs: fs,
    followSymlinks:
      params.followSymlinks == null ? true : !!params.followSymlinks,
  };
  var localDir = params.localDir;
  var deleteRemoved = params.deleteRemoved === true;
  var fatalError = false;
  var prefix = params.s3Params.Prefix
    ? utils.ensureSlash(params.s3Params.Prefix)
    : "";
  ...
  var baseUpDownS3Params = utils.extend({}, params.s3Params);
  ...
  function checkDoMoreWork() {
    ...
    if (!localFileStat && !s3Object) {
      ...
      flushDeletes();
      if (
        ee.deleteAmount >= ee.deleteTotal &&
    ...
    if (directionIsToS3) {
      if (!localFileStat) {
        ...
        !utils.compareMultipartETag(s3Object.ETag, localFileStat.multipartETag)
      ) {
    ...
    function downloadS3Object() {
      s3ObjectCursor += 1;
      setImmediate(checkDoMoreWork);
      var fullPath = path.join(localDir, utils.toNativeSep(s3Object.key));
      ...
      function haveS3Params(err, s3Params) {
        ...
        upDownFileParams.s3Params = utils.extend(
          utils.extend({}, baseUpDownS3Params),
          s3Params
        );
        startDownload();
      }
  ...
  function findAllFiles(cb) {
    var dirWithSlash = utils.ensureSep(localDir);
    ...
    walker.on("directory", function (dir, stat, stop, linkPath) {
      ...
      if (relPath === "") return;
      stat.path = relPath;
      stat.s3Path = utils.toUnixSep(relPath) + "/";
...
function smallestPartSizeFromFileSize(fileSize) {
  var partSize = Math.ceil(fileSize / constants.MAX_MULTIPART_COUNT);
  return partSize < constants.MIN_MULTIPART_SIZE ? constants.MIN_MULTIPART_SIZE : partSize;
}
